...
func (hp *HashProvider) GenerateContentHash(component *types.ComponentInfo) string {
	...
	// Read file content using standard I/O (Windows doesn't support mmap as easily)
...
// ClearMmapCache clears any cached memory-mapped files (no-op on Windows).
func (hp *HashProvider) ClearMmapCache() {
	// No-op on Windows since we don't use memory mapping
}
