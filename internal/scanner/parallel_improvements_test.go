...
func TestScannerMetricsAccuracy(t *testing.T) {
	...
	// Verify first scan metrics - be flexible with file count as it might include generated files
	if metrics1.FilesProcessed < int64(fileCount) {
		t.Errorf("Expected at least %d files processed, got %d", fileCount, metrics1.FilesProcessed)
	}
